...
import { orderApi, OrderApiError } from '@/services/orderApi';
  ...
  export const orderKeys = {
    ...
    detailByCode: (code: string) => [...orderKeys.details(), 'code', code] as const,
    userOrders: (userId?: number) => [...orderKeys.all, 'user', userId] as const,
    search: (params: OrderSearchParams) => [...orderKeys.all, 'search', params] as const,
    admin: {
      all: () => [...orderKeys.all, 'admin'] as const,
      details: (id: number) => [...orderKeys.admin.all(), 'details', id] as const,
      detailsByCode: (code: string) => [...orderKeys.admin.all(), 'details', 'code', code] as const,
    },
  };
  ...
  export const useOrderManagement = () => {
    ...
    const searchQuery = useSearchOrders(
      ...
      { enabled: !!orderFilters.searchTerm }
    );
  
    // Return search results if searching, otherwise return all orders
    const orders = orderFilters.searchTerm ? searchQuery.data || [] : ordersQuery.data || [];
    
    return {
      orders,
      isLoading: ordersQuery.isLoading || searchQuery.isLoading,
      error: ordersQuery.error || searchQuery.error,
      refetch: () => {
        ordersQuery.refetch();
  ...
  export function useCheckoutProcess() {
    const mutation = useMutation({
      mutationFn: async ({ user_id, items }: CheckoutPayload) => {
        const baseUrl =
    ...
    return {
      ...
      checkoutError: mutation.error ? (mutation.error as Error).message : null,
    };
  }
